	...
	<div id="content">
		...
		<div id="middle">
			<div class="listnav">
				...
				<a target='_blank' href='daochu.php?uid=<?PHP echo $uid;?>' class='daochu'>
					导出确认论文</a>
			</div>
				...
				<div class='paperlist'>
					<?PHP
						if($totalnum == 0){
							...
							echo "<table><tr><th width=60>序号</th>
								<th>论文题目</th><th width=60>年份</th>
								<th width=40>编辑</th><th width=40>删除</th></tr>";
					
							$query = "select id,date,title from paper where (uid=$uid or stid=$tid) and (uright='right' or 
								(uright='notconfirm' and(isright=5 or vipright=5 or cnkiright=5))) order by date desc limit $start,$pagesize";
							...
							while($row = mysql_fetch_array($result)){
								...
								echo "<tr id='".$row['id']."'><td>$j</td><td><a class='prev' title='点击查看详细信息' 
									...
									$BASEURL."/viewpaper/editpaper.php?pid=".$row['id']."'><img src='img/icon_edit.gif' title='编辑'></a></td>
									<td id='del$pid'><img style='cursor:pointer' src='img/icon_del.gif' title='删除'  onclick='delPaper(".
									$row['id'].")'></td></tr>";
							}
					...
					<div id="pageTopPanel" class="float-right">
						...
						<?php
							if($totalnum>0){
								...
								//打印上一页
								if($start-$pagesize>=0)
									echo "<li><a href='$BASEURL/viewpaper/truepaperlist.php?uid=$uid&start=".
								...
								for($i=$startindex,$j=0;$i<$totalnum && $j<$pagelinknum;$i = $i+$pagesize,$j++){					
									if($i==$start)
										echo "<li class='current'><a href='$BASEURL/viewpaper/truepaperlist.php?uid=$uid&start=$start'>$p</a></li>";
									else{
										echo "<li><a href='$BASEURL/viewpaper/truepaperlist.php?uid=$uid&start=$i'>$p</a></li>";
									}
								...
								//打印下一页链接
								if($start+$pagesize<$totalnum)
									echo "<li><a href='$BASEURL/viewpaper/truepaperlist.php?uid=$uid&start=".($start+$pagesize)."'>下一页</a></li>";
							}
						?>
						</ol>
					</div>
